...
// QA de tokens (referência) — movida de app/page.tsx para /qa no Prompt 2.
// Mantida para validar tokens da TRIBUS_LABS_DESIGN.md: escala tipográfica (§3),
// cores semânticas (§2), botões (§9) e o toggle. Acesse em /qa.
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return <p className="text-eyebrow mb-3 text-accent">{children}</p>;
}
    ...
    <div className="min-h-full">
      ...
      <main className="mx-auto flex max-w-5xl flex-col gap-20 px-6 py-16">
            ...
            {colorTokens.map((c) => (
              ...
              >
                ...
                <div className="flex flex-col gap-1 p-3">
                  ...
                  <span className="text-caption text-text-muted">dark {c.dark}</span>
                  <span className="text-caption text-text-muted">light {c.light}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <footer className="text-caption text-text-muted">
          QA de referência — TRIBUS_LABS_DESIGN.md v1.0.
        </footer>
      </main>
    </div>
  );
